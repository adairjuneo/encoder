...
import type { FFmpegService } from '../layers/FFmpegLayer.js';
...
export function transcodeJob(
  ...
  return Effect.gen(function* () {
    ...
    const batches: Resolution[][] = Arr.chunksOf(
      RESOLUTIONS,
      config.RESOLUTION_BATCH_SIZE,
    );
      ...
      const results = yield* Effect.forEach(
        ...
        (resolution) =>
          transcodeResolution(
            ...
            workDir,
            jobState,
          ).pipe(Effect.map((progress) => ({ resolution, progress }))),
    ...
    const [thumbnailKey] = yield* Effect.all(
      [
        ...
        (yield* R2Service).uploadBuffer(
          masterKey,
          Buffer.from(masterLines, 'utf-8'),
        ),
      ],
      { concurrency: 2 },
    );
    ...
    const costReport = yield* cost.calculateJobCost(
      instanceType,
      startedAt,
      endedAt,
    );
